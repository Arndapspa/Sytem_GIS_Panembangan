                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="d-block bg-light p-3 custom-rounded-medium" v-if="showDetail">
                                            <div class="d-flex justify-content-between mb-4">
                                                <h5 class="mt-2">Detail Kepemilikan</h5>
...
import { doc, addDoc, getDocs, getDoc, collection, query, where, deleteDoc, limit } from "firebase/firestore";
...
export default {
        ...
        ownerName(value) {
            if (value != this.ownerSelected.name) {
                this.polygons.forEach(element => {
                    element.setStyle({
                        color: "green", // Warna garis tepi
                        fillColor: "rgba(0, 102, 102, 1)", // Warna isian (dengan transparansi)
                    });
                });
            }
        },
    ...
    methods: {
        ...
        async initMap() {
            ...
            }).setView([this.detailKopak.latitude, this.detailKopak.longitude], 20);
            ...
            if (this.allPolygons) {
                this.allPolygons.forEach(element => {
                    ...
                    polygon.on("click", (e) => {
                        this.polygonClicked = e.target;
                        this.showDetail = true
                        this.detailMap = element
            ...
            this.initialMap.on("click", this.onMapClick);
        },
        setSelectedPolygon() {
            ...
            this.polygonClicked.setStyle({
                ...
                fillColor: "rgba(255, 0, 0, 0.3)", // Ubah warna isi menjadi merah dengan transparansi
            });
        ...
        onMapClick(e) {
            ...
            // Cek apakah titik yang diklik sudah ada dalam salah satu polygon
            const isInsidePolygon = this.polygons.some((polygon) =>
                polygon.getBounds().contains(clickedLatLng)
            );
            ...
            // Jika tidak berada dalam polygon yang ada, lanjutkan menggambar garis
            this.points.push([clickedLatLng.lat, clickedLatLng.lng]);

            // Perbarui atau buat garis baru
            this.updatePolyline();

            // Jika sudah 4 titik, tutup garis dan reset
            if (this.points.length === 4) {
                this.completePolygon();
            }
        },
        updatePolyline() {
            // Jika garis sudah ada, hapus dari peta
            if (this.polyline) {
                this.initialMap.removeLayer(this.polyline);
            }

            // Buat garis baru berdasarkan titik yang ada
            this.polyline = L.polyline(this.points, { color: "green" }).addTo(this.initialMap);
        },
        completePolygon() {
            // Tambahkan titik awal ke akhir array untuk menutup kotak
            ...
            this.allPolygons.push(dataPolygon)
            ...
            // Gambar polygon dengan background
            ...
            this.saveLocationToDatabase(dataPolygon)

            // Simpan polygon ke dalam array
            this.polygons.push(this.polygon);

            // Menambahkan event listener pada polygon
            this.polygon.on("click", (e) => {
                this.polygonClicked = e.target
                const polygonId = e.target.options.id;

                this.setSelectedPolygon()
                this.showDetail = true

                this.detailMap = find(this.allPolygons, {id: polygonId})
            });

            // Reset points dan polygon
            this.points = [];
            this.polyline = null;
            
        },
        async saveLocationToDatabase(data) {
            try {
                this.fetch = true
                this.loading = this.$loading.show()
